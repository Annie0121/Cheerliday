...
export function Schedule({record,setSelectedDay,travelTimes,setRecord,isAuthor}:Schedule){
    ...
    const onDragEnd =async(event:any)=>{
      ...
      //拷貝資料
      const newDateRange = [...record.dateRange];
      const sourceDateIndate = source.droppableId
      const sourceAttractionIndex = source.index
      const destinationDateIndex =destination.droppableId
      const destinationAttractionIndex =destination.index

      //處理拖曳後資料
      const [remove] = newDateRange[sourceDateIndate].attractions.splice(sourceAttractionIndex,1)
      newDateRange[destinationDateIndex].attractions.splice(destinationAttractionIndex,0,remove)
    ...
    return(
        <div >
            ...
            <div style={{backgroundColor:'#efefefff',paddingTop:'20PX',paddingBottom:'40px'}}> 
             ...
             <DragDropContext onDragEnd={onDragEnd} onDragStart={()=>{setAutoScrollEnabled(false);}}>
                {dateRange.map((date,dateindex)=>{
                   const times = calculateTimes(date);
                    ...
                    <React.Fragment key={dateindex} >
                        ...
                        <div style={{display:'flex',margin:'35px 10px 0 10px'}}>
                             {/*行程的日期、出發時間等*/}
                            <div 
                         ...
                         {/*該日期的景點名稱順序*/}
                         <Droppable droppableId={dateindex.toString()}>
                                {(provided, snapshot) => (
                                  <div {...provided.droppableProps} ref={provided.innerRef} style={{margin:'35px 0'}}>
